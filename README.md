...
Perhaps it has caused me to become overly biased in favor of hiding (via display:none) rather than deleting children when no longer needed at least for the moment, in order to hold on to their "memories".  This might not always be the right choice, especially on a low-memory device.  Polymer got that right -- [support both](https://polymer-library.polymer-project.org/2.0/docs/devguide/templates#dom-if).  It makes a world of a difference, in my experience, in a desktop setting.  But I'm afraid I've fallen short in thinking outside my use cases.  I will resolve to follow their lead on this front, despite my reluctance caused by my bias.
